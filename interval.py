...
def est(mean, std, sample_size, CI):
    mean = mean
    std = std
    ...
    if(sample_size < 30):
            ...
            try:
                ...
                lrange = mean - (t_alpha*(std/math.sqrt(sample_size)))
                urange = mean + (t_alpha*(std/math.sqrt(sample_size)))
            except Exception as e:
                print(e)

            print("This is a t-distribution with the")
            print(f"PE = {mean}")
            print(f"IE = {lrange} to {urange}")
            ...
            try:
                ...
                lrange = mean - (z*(std/math.sqrt(sample_size)))
                urange = mean + (z*(std/math.sqrt(sample_size)))
                print("This is a z-distribution with the")
                print(f"PE = {mean}")
                print(f"IE = {lrange} to {urange}")
            except Exception as e:
                print(e)

def main(csv, mean, std, sample_size, CI):
    csv = csv
    mean = mean
    std = std
    sample_size = sample_size
